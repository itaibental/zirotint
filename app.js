...
/* ---------- Slide reading-lock (30s + tap-to-confirm) ---------- */
const SLIDE_LOCK_SECONDS = 30;
let slideLockInterval = null;
let slideLockSecondsLeft = SLIDE_LOCK_SECONDS;
...
/* ---------- Slide reading-lock logic ---------- */
function clearSlideLock() {
  if (slideLockInterval) {
    clearInterval(slideLockInterval);
    slideLockInterval = null;
  }
}

function updateSlideLockBanner() {
  const banner = document.getElementById('slideLockBanner');
  const textEl = document.getElementById('slideLockText');
  const iconEl = document.getElementById('slideLockIcon');
  if (!banner || !textEl || !iconEl) return;

  if (!slideLockTouched) {
    iconEl.innerText = '👉';
    textEl.innerHTML = `יש לגעת בטקסט השקף ולקרוא אותו · נותרו <span id="slideLockSeconds">${slideLockSecondsLeft}</span> שניות`;
  } else {
    iconEl.innerText = '⏳';
    textEl.innerHTML = `תודה! ממשיכים לקרוא · נותרו <span id="slideLockSeconds">${slideLockSecondsLeft}</span> שניות עד שתוכלו להמשיך`;
  }
}

function checkSlideLockRelease() {
  if (slideLockActive && slideLockSecondsLeft <= 0 && slideLockTouched) {
    releaseSlideLock();
  }
}
...
function releaseSlideLock() {
  ...
  if (dotsCont) {
    dotsCont.classList.remove('opacity-40', 'pointer-events-none');
  }
  if (banner) {
    banner.innerHTML = `<span class="text-lg shrink-0">✅</span><span>קראתם את השקף - אפשר להמשיך לעמוד הבא</span>`;
    setTimeout(() => {
      if (!slideLockActive) banner.classList.add('hidden');
    }, 1500);
  }
}

function onSlideContentTouched() {
  if (!slideLockActive || slideLockTouched) return;
  slideLockTouched = true;
  updateSlideLockBanner();
  checkSlideLockRelease();
}

function startSlideLock() {
  clearSlideLock();
  slideLockSecondsLeft = SLIDE_LOCK_SECONDS;
  slideLockTouched = false;
  slideLockActive = true;
  ...
  if (dotsCont) {
    dotsCont.classList.add('opacity-40', 'pointer-events-none');
  }
  if (banner) {
    // בונים מחדש את מבנה הבאנר הפנימי בכל פעם, כי הצגת סיום ההשהיה (releaseSlideLock)
    // מחליפה את תוכנו הפנימי - כך מובטח שהאלמנטים הפנימיים תמיד קיימים בתחילת שקף חדש.
    banner.innerHTML = `
      <span class="text-lg shrink-0" id="slideLockIcon">👉</span>
      <span id="slideLockText"></span>
    `;
    banner.classList.remove('hidden');
  }
  updateSlideLockBanner();

  slideLockInterval = setInterval(() => {
    if (slideLockSecondsLeft > 0) {
      slideLockSecondsLeft--;
      updateSlideLockBanner();
    }
    checkSlideLockRelease();
  }, 1000);
...
function renderCurrentSlide() {
  ...
  if (currentSlideIndex === slides.length - 1) {
    ...
    nextBtn.className = "px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-black text-xs sm:text-sm transition flex items-center gap-1.5 shadow-md shadow-indigo-600/20 disabled:opacity-40 disabled:pointer-events-none";
    nextBtn.onclick = () => {
      playSound('click');
      currentSlideIndex++;
      renderCurrentSlide();
